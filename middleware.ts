...
export function middleware(request: NextRequest) {
    ...
    // Protect admin and superadmin routes: redirect to /portal-campuseye3x101 if no token
    if ((pathname.startsWith("/admin") || pathname.startsWith("/superadmin")) && !token) {
        const loginUrl = new URL("/portal-campuseye3x101", request.url);
    ...
    if (pathname === "/portal-campuseye3x101" && token) {
...
export const config = {
    matcher: ["/admin/:path*", "/superadmin/:path*", "/portal-campuseye3x101"],
};
